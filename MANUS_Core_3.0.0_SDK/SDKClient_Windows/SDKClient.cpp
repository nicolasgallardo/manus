...
//TODO: set the correct com interface for your setup
surge_Comm leftHand(L"COM8");
surge_Comm rightHand(L"COM7");
...
ClientReturnCode SDKClient::DisplayingData()
{
	...
	ClientLog::print("[R] Run Surge");
	ClientLog::print("[B] Calibrate Surge");
	...
	GO_TO_DISPLAY('R', DisplayingRunSurge);
	GO_TO_DISPLAY('B', DisplayingCalibrateSurge);
...
ClientReturnCode SDKClient::DisplayingRunSurge()
{
	ClientLog::print("[Q] Back  <<Run Surge>> [ESC] quit [L] Toggle Logging");
	...
	if (GetKeyDown('L')) {
		if (leftHand.logging) {
			leftHand.logging = false;
		}
		else {
			leftHand.logging = true;
		}
		if (rightHand.logging) {
			rightHand.logging = false;
		}
		else {
			rightHand.logging = true;
		}
	}

	if (leftHand.logging || rightHand.logging) {
		ClientLog::print("Logging = True");
	}
	else {
		ClientLog::print("Logging = False");
	}
	...
	{
		RunSurge(m_LeftGloveErgoData, true);
	}
	else
	{
	...
	{
		RunSurge(m_RightGloveErgoData, false);
	}
	else
	{
		ClientLog::print(" ...No Data...");
...
ClientReturnCode SDKClient::DisplayingCalibrateSurge()
{
	ClientLog::print("[Q] Back  <<Surge Calibration>> [ESC] quit");

	AdvanceConsolePosition(2);

	GO_TO_MENU_IF_REQUESTED();

	ClientLog::print("Follow the link below to calibrate min and max positions.");
	AdvanceConsolePosition(1);
	ClientLog::print("[insert YouTube link]");
	AdvanceConsolePosition(2);
	ClientLog::print("Press [R] to begin calibration of right hand. Press [L] to begin calibration of left hand.");
	AdvanceConsolePosition(2);
	if (GetKeyDown('R')) {
		if (rightHand.calibrate) {
			rightHand.calibrate = false;
		}
		else {
			rightHand.calibrate = true;
		}
	}
	if (GetKeyDown('L')) {
		if (leftHand.calibrate) {
			leftHand.calibrate = false;
		}
		else {
			leftHand.calibrate = true;
		}
	}
	if (rightHand.calibrate) {
		float angles[6] = {0.0};
		float final[12] = { 0.0 };
		ClientLog::print("Right hand calibration = True");
		AdvanceConsolePosition(1);
		ClientLog::print("Step 1: Hold in open position for 5 seconds");
		std::this_thread::sleep_for(std::chrono::milliseconds(5000));
		getErgo(m_RightGloveErgoData, angles, false);
		std::cout << "TR: " << angles[0] << std::endl;
		std::cout << "TF: " << angles[1] << std::endl;
		std::cout << "I: " << angles[2] << std::endl;
		std::cout << "M: " << angles[3] << std::endl;
		std::cout << "R: " << angles[4] << std::endl;
		std::cout << "P: " << angles[5] << std::endl;
		//save the mins
		final[0] = angles[0];
		final[1] = angles[1];
		final[2] = angles[2];
		...
		angles[4] = 0.0;
		angles[5] = 0.0;
		AdvanceConsolePosition(7);
		ClientLog::print("DONE!");
		AdvanceConsolePosition(1);
		ClientLog::print("Step 2: Hold in fist for 5 seconds");
		std::this_thread::sleep_for(std::chrono::milliseconds(5000));
		getErgo(m_RightGloveErgoData, angles, false);
		std::cout << "TR: " << angles[0] << std::endl;
		std::cout << "TF: " << angles[1] << std::endl;
		...
		ClientLog::print("DONE!");
		AdvanceConsolePosition(1);
		ClientLog::print("Step 3: Touch thumb to palm for 5 seconds");
		std::this_thread::sleep_for(std::chrono::milliseconds(5000));
		getErgo(m_RightGloveErgoData, angles, false);
		std::cout << "TR: " << angles[0] << std::endl;
		std::cout << "TF: " << angles[1] << std::endl;
		...
		final[6] = angles[0];
		final[7] = angles[1];
		rightHand.sendCalibration(final);
		AdvanceConsolePosition(7);
		ClientLog::print("DONE!");
		AdvanceConsolePosition(1);
		rightHand.calibrate = false;
		...
		ClientLog::print("Right hand calibration = False");
	}
	AdvanceConsolePosition(1);
	if (leftHand.calibrate) {
		float angles[6] = { 0.0 };
		float final[12] = { 0.0 };
		ClientLog::print("Left hand calibration = True");
		AdvanceConsolePosition(1);
		ClientLog::print("Step 1: Hold in open position for 5 seconds");
		std::this_thread::sleep_for(std::chrono::milliseconds(5000));
		getErgo(m_LeftGloveErgoData, angles, true);
		std::cout << "TR: " << angles[0] << std::endl;
		std::cout << "TF: " << angles[1] << std::endl;
		std::cout << "I: " << angles[2] << std::endl;
		std::cout << "M: " << angles[3] << std::endl;
		...
		angles[4] = 0.0;
		angles[5] = 0.0;
		leftHand.sendCalibration(final);
		AdvanceConsolePosition(1);
		ClientLog::print("DONE!");
		AdvanceConsolePosition(1);
		leftHand.calibrate = false;
		...
		ClientLog::print("Left hand calibration = False");
	}
...
//-----------------------------------------Alt-Bioinics: SERIAL--------------------------------------------------
//***************************************************************************************************************
void SDKClient::getErgo(ErgonomicsData& p_ErgoData, float * angles, bool p_Left) {
	...
	{
		if (p_Left) {
			if (t_FingerNames[t_FingerNumber] == "[thumb] ") {
				//TR = RoundFloatValue(p_ErgoData.data[t_DataOffset], 2);
				TR = RoundFloatValue(p_ErgoData.data[t_DataOffset + 1], 2);
				TF = RoundFloatValue(p_ErgoData.data[t_DataOffset + 3], 2);
			}
			if (t_FingerNames[t_FingerNumber] == "[index] ") {
				I = RoundFloatValue(p_ErgoData.data[t_DataOffset + 2], 2);
			}
			if (t_FingerNames[t_FingerNumber] == "[middle]") {
				M = RoundFloatValue(p_ErgoData.data[t_DataOffset + 2], 2);
			}
			if (t_FingerNames[t_FingerNumber] == "[ring]  ") {
				R = RoundFloatValue(p_ErgoData.data[t_DataOffset + 2], 2);
			}
			if (t_FingerNames[t_FingerNumber] == "[pinky] ") {
				P = RoundFloatValue(p_ErgoData.data[t_DataOffset + 2], 2);
			}
		}
		else {
			if (t_FingerNames[t_FingerNumber] == "[thumb] ") {
				//TR = RoundFloatValue(p_ErgoData.data[t_DataOffset], 2);
				TR = RoundFloatValue(p_ErgoData.data[t_DataOffset + 1], 2);
				TF = RoundFloatValue(p_ErgoData.data[t_DataOffset + 2], 2);
			}
			if (t_FingerNames[t_FingerNumber] == "[index] ") {
				I = RoundFloatValue(p_ErgoData.data[t_DataOffset + 2], 2);
			}
...
void SDKClient::RunSurge(ErgonomicsData& p_ErgoData, bool p_Left) {
	const std::string t_FingerNames[NUM_FINGERS_ON_HAND] = { "[thumb] ", "[index] ", "[middle]", "[ring]  ", "[pinky] " };
	const std::string t_FingerJointNames[NUM_FINGERS_ON_HAND] = { "mcp", "pip", "dip" };
	const std::string t_ThumbJointNames[NUM_FINGERS_ON_HAND] = { "cmc", "mcp", "ip " };
	...
	for (unsigned int t_FingerNumber = 0; t_FingerNumber < NUM_FINGERS_ON_HAND; t_FingerNumber++)
	{
		if (p_Left) {
			printf("LEFT: ");
			if (t_FingerNames[t_FingerNumber] == "[thumb] ") {
				//float TR = RoundFloatValue(p_ErgoData.data[t_DataOffset], 2);
				float TR = RoundFloatValue(p_ErgoData.data[t_DataOffset + 1], 2);
				//***************************************************************************************************************
				//-----------------------------------------Alt-Bioinics: SERIAL--------------------------------------------------
				//***************************************************************************************************************
				leftHand.sendString("X", TR, p_Left);
				//***************************************************************************************************************
				//float TF = RoundFloatValue(p_ErgoData.data[t_DataOffset + 2], 2);
				float TF = RoundFloatValue(p_ErgoData.data[t_DataOffset + 3], 2);
				//***************************************************************************************************************
				//-----------------------------------------Alt-Bioinics: SERIAL--------------------------------------------------
				//***************************************************************************************************************
				printf("LEFT: ");
				leftHand.sendString("T", TF, p_Left);

			}
			if (t_FingerNames[t_FingerNumber] == "[index] ") {
				float I = RoundFloatValue(p_ErgoData.data[t_DataOffset + 2], 2);
				//***************************************************************************************************************
				//-----------------------------------------Alt-Bioinics: SERIAL--------------------------------------------------
				//***************************************************************************************************************
				leftHand.sendString("I", I, p_Left);

			}
			if (t_FingerNames[t_FingerNumber] == "[middle]") {
				float M = RoundFloatValue(p_ErgoData.data[t_DataOffset + 2], 2);
				//***************************************************************************************************************
				//-----------------------------------------Alt-Bioinics: SERIAL--------------------------------------------------
				//***************************************************************************************************************
				leftHand.sendString("M", M, p_Left);

			}
			if (t_FingerNames[t_FingerNumber] == "[ring]  ") {
				float R = RoundFloatValue(p_ErgoData.data[t_DataOffset + 2], 2);
				//***************************************************************************************************************
				//-----------------------------------------Alt-Bioinics: SERIAL--------------------------------------------------
				//***************************************************************************************************************
				leftHand.sendString("R", R, p_Left);

			}
			if (t_FingerNames[t_FingerNumber] == "[pinky] ") {
				float P = RoundFloatValue(p_ErgoData.data[t_DataOffset + 2], 2);
				//***************************************************************************************************************
				//-----------------------------------------Alt-Bioinics: SERIAL--------------------------------------------------
				//***************************************************************************************************************
				leftHand.sendString("P", P, p_Left);
			...
			if (t_FingerNames[t_FingerNumber] == "[thumb] ") {
				...
				//-----------------------------------------Alt-Bioinics: SERIAL--------------------------------------------------
				//***************************************************************************************************************
				rightHand.sendString("X", TR, p_Left);
				...
				//-----------------------------------------Alt-Bioinics: SERIAL--------------------------------------------------
				//***************************************************************************************************************
				rightHand.sendString("T", TF, p_Left);
			...
			if (t_FingerNames[t_FingerNumber] == "[index] ") {
				...
				//-----------------------------------------Alt-Bioinics: SERIAL--------------------------------------------------
				//***************************************************************************************************************
				rightHand.sendString("I", I, p_Left);
			...
			if (t_FingerNames[t_FingerNumber] == "[middle]") {
				...
				//-----------------------------------------Alt-Bioinics: SERIAL--------------------------------------------------
				//***************************************************************************************************************
				rightHand.sendString("M", M, p_Left);
			...
			if (t_FingerNames[t_FingerNumber] == "[ring]  ") {
				...
				//-----------------------------------------Alt-Bioinics: SERIAL--------------------------------------------------
				//***************************************************************************************************************
				rightHand.sendString("R", R, p_Left);
			...
			if (t_FingerNames[t_FingerNumber] == "[pinky] ") {
				...
				//-----------------------------------------Alt-Bioinics: SERIAL--------------------------------------------------
				//***************************************************************************************************************
				rightHand.sendString("P", P, p_Left);
